...
migrate(Pause) ->
    ...
    migrate_config_setting(
      ...
      ,{<<"stepswitch">>, <<"ensure_valid_emergency_number">>}
     ),
    ...
    io:format("moving callflows.default_emergency_cid_number to stepswitch.default_emergency_cid_number...~n"),
    _ = stepswitch_maintenance:migrate(Accounts),
...
%%  exists and will move per-node settings if they exist.  It can
%%  handle moving complex or nested keys.
...
%% @end
%%--------------------------------------------------------------------
-type migrate_setting() :: {ne_binary(), ne_binary()}.
-type migrate_value() :: {ne_binary(), ne_binary(), ne_binary() | ne_binaries(), _}.
...
-spec add_config_setting(ne_binary(), ne_binary(), migrate_values()) ->
                                'ok' |
                                {'error', _}.
...
add_config_setting(JObj, ToSetting, [{FromId, Node, FromSetting, Value} | Values]) ->
    ...
    case wh_json:get_value(Key, JObj) of
        ...
        _Else ->
            io:format("the system tried to move the parameter listed bellow but found a different setting already there, you need to correct this disparity manually!~n", []),
...
-spec remove_config_setting(migrate_setting()) ->
                                   {'ok', wh_json:object(), wh_proplist()} |
                                   {'error', _}.
remove_config_setting({Id, Setting}) ->
    remove_config_setting(Id, Setting).

-spec remove_config_setting(ne_binary() | wh_json:object(), ne_binary() | ne_binaries()) ->
                                   {'ok', wh_json:object(), wh_proplist()} |
                                   {'error', _}.
...
-spec remove_config_setting(wh_proplist(), wh_json:object(), migrate_values()) ->
...
-spec config_setting_key(ne_binary(), ne_binary() | ne_binaries()) -> ne_binaries().
