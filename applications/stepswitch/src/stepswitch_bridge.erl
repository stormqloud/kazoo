...
handle_cast({'gen_listener', {'is_consuming', 'true'}}, State) ->
    case maybe_bridge(State) of
        'ok' ->
            lager:debug("sent bridge command to ~s", [State#state.control_queue]),
            {'noreply', State};
        _Else -> {'stop', 'normal', State}
    end;
...
%%% Internal functions
%%%===================================================================
-spec maybe_bridge(state()) -> 'ok' | 'stop'.
...
-spec maybe_bridge_emergency(state()) -> 'ok' | 'stop'.
maybe_bridge_emergency(#state{resource_req=JObj
    ...
    case wh_json:get_value(<<"Hunt-Account-ID">>, JObj) of
        'undefined' ->
            maybe_terminate_emergency_bridge(State, Number, Name);
        _Else ->
...
-spec maybe_terminate_emergency_bridge(state(), api_binary(), api_binary()) -> 'ok' | 'stop'.
maybe_terminate_emergency_bridge(State, 'undefined', Name) ->
    case whapps_config:get_is_true(
           <<"stepswitch">>
           ,<<"terminate_unconfigured_emergency">>
           ,'false'
          )
    of
        'false' ->
            maybe_terminate_emergency_bridge(
              State
              ,default_emergency_number(wh_util:anonymous_caller_id_number())
              ,Name
             );
        'true' -> terminate_emergency_bridge(State)
    end;
maybe_terminate_emergency_bridge(#state{control_queue=ControlQ}=State, Number, Name) ->
    wapi_dialplan:publish_command(
      ControlQ
      ,build_bridge(State, Number, Name)
     ).
